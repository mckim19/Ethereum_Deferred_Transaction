...
func opLock(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	param:= stack.pop().Uint64()
    msg:=types.ChanMsg{
    	TxHash: interpreter.evm.Context.YMTxHash, ContractAddr: contract.Address(), 
    	LockName: param, LockType:"LOCK", IsLockBusy: false, Channel: make(chan types.ChanMsg, 10),
    }
    ch_com <- msg 
...
func opUnlock(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	msg:=types.ChanMsg{
		TxHash: interpreter.evm.Context.YMTxHash, ContractAddr: contract.Address(), 
		LockName: param, LockType:"UNLOCK", IsLockBusy: false, Channel: make(chan types.ChanMsg,10),
	}
	ch_com<-msg
