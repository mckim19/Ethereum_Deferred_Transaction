...
func (q *queue) Results(block bool) []*fetchResult {
	...
	if len(results) > 0 {
		...
		for _, result := range results {
			...
			*/
			for _, recInfo := range result.RecInfos {
				size += recInfo.Size()
			}
...
/*
	OSDC Parallel. Yoomee Ko.
*/
func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction, uncleLists [][]*types.Header, recInfoLists [][]*types.RecInfo) (int, error) {
//func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction, uncleLists [][]*types.Header) (int, error) {
	...
	reconstruct := func(header *types.Header, index int, result *fetchResult) error {
		...
		result.RecInfos = recInfoLists[index]
		return nil
	}
