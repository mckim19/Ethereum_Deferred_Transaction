...
func (d *Downloader) fetchBodies(from uint64) error {
	...
	var (
		deliver = func(packet dataPack) (int, error) {
			...
			return d.queue.DeliverBodies(pack.peerID, pack.transactions, pack.uncles, pack.recInfos)
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		...
		*/
		blocks[i] = types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles, result.RecInfos)
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	/*
		OSDC Parallel. Yoomee Ko.
	*/
	block := types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles, result.RecInfos)
...
func (d *Downloader) DeliverBodies(id string, transactions [][]*types.Transaction, uncles [][]*types.Header, recInfos [][]*types.RecInfo) (err error) {
	return d.deliver(id, d.bodyCh, &bodyPack{id, transactions, uncles, recInfos}, bodyInMeter, bodyDropMeter)
}
